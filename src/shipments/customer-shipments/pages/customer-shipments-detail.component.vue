<template>
  <div class="detail-main-content" role="main">
    <router-link
      :to="`/customers/${this.customerId}/shipments`"
      class="no-underline"
      ><pv-button
        label="Go back"
    ...
    <div class="grid-list" role="contentinfo">
      <pv-card class="card-info">
        ...
        <template #title>
          <p class="text-center">
            <i class="pi pi-user mr-5 text-4xl"></i>{{ this.enterprise.name }}
          </p>
        </template>
        <template #content>
          <p class="text-center">
            <span class="font-bold">Phone Number: </span
            >{{ this.enterprise.phoneNumber }}
          </p>
          <p class="text-center">
            <span class="font-bold">Email: </span>{{ this.enterprise.email }}
          </p>
          <p class="text-center">
            <span class="font-bold">Description: </span
            >{{ this.enterprise.serviceDescription }}
          </p>
          <p class="text-center">
            <span class="font-bold">Ruc: </span>{{ this.enterprise.ruc }}
          </p>
        </template>
      </pv-card>
      <pv-card class="card-info">
        ...
        <template #content>
          <pv-timeline :value="customerShipmentEvents">
            <template #marker="slotProps">
              <span
                v-if="slotProps.item === customerShipment.status"
                class="status-circle"
        ...
        <template #content>
          <div class="w-full flex flex-row justify-content-between flex-wrap">
            <p>
              ...
              >{{ this.customerShipment.origin }}
            </p>
            <p>
              <span class="font-bold">Pick up date: </span
              >{{ this.customerShipment.pickUpDate }}
            </p>
          </div>
          <div class="w-full flex flex-row justify-content-between flex-wrap">
            <p>
              <span class="font-bold">Destiny: </span
              >{{ this.customerShipment.destiny }}
            </p>
            <p>
              <span class="font-bold">Delivery Date: </span
              >{{ this.customerShipment.deliveryDate }}
            </p>
          </div>
          <div class="w-full flex flex-row justify-content-between flex-wrap">
