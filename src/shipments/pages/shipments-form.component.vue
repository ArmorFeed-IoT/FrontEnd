  ...
  >
    <div v-for="(field, index) in fields" class="field" v-bind:key="index">
      ...
      <div v-if="field.type === 1" class="field">
        <pv-input-text
          v-model.trim="shipment[field.name]"
          required="true"
          autofocus
          :class="{ 'p-invalid': submitted && !shipment[field.name] }"
        />
        <small class="p-error" v-if="submitted && !shipment[field.name]"
  ...
  computed: {
    isValid: function () {
      ...
      this.fields.forEach((field) => {
        if (!this.shipment[field.name]) band = false;
      });
      return band;
    },
  },
  methods: {
    handleSubmit() {
      if (this.shipment["pickUpDate"])
        this.shipment["pickUpDate"] =
          this.shipment["pickUpDate"].toLocaleString("en-GB");
      if (this.shipment["deliveryDate"])
        this.shipment["deliveryDate"] =
          this.shipment["deliveryDate"].toLocaleString("en-GB");
      this.$emit("submit");
    },
  },
};
